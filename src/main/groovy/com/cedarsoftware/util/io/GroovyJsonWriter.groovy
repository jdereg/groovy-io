...
class GroovyJsonWriter implements Closeable, Flushable
{
     ...
     */
    public GroovyJsonWriter(OutputStream out) throws IOException
    {
        this(out, [:])
    }
     ...
     */
    public GroovyJsonWriter(OutputStream out, Map<String, Object> optionalArgs) throws IOException
    {
        Map args = _args.get()
        args.clear()
...
//        boolean typeWritten = showType && !(Object[].class == arrayType)    // causes IDE warning in NetBeans 7/4 Java 1.7
    ...
    private void writeJsonObjectArray(JsonObject jObj, boolean showType) throws IOException
    {
        ...
        if (referenced)
        {
            writeId(Long.toString(jObj.getId()))
            output.write(',')
            newLine()
        }
    ...
    private void writeJsonObjectObject(JsonObject jObj, boolean showType) throws IOException
    {
        ...
        if (showType)
        {
            ...
            output.write('"@type":"')
            output.write(jObj.getType())
            output.write('"')
            try  { type = GroovyJsonReader.classForName(jObj.getType()) }
